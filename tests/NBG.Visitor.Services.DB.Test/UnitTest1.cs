...
namespace NBG.Visitor.Services.DB.Test
{
    public class Tests
    {
        ...
        {
            var visitor = _repo.GetVisitorIfExists("Pat", "Du Plantier", "724 964 6781");
            Assert.IsInstanceOf<Storage.Models.Visitor>(visitor);
            visitor = _repo.GetVisitorIfExists("Non", "existent", "person");
        ...
        public async Task TestAddVisit()
        {
            ...
            var visitor = _repo.GetVisitorIfExists("Dietfried", "Haber", "0664 1234567");
            Assert.IsInstanceOf<Storage.Models.Visitor>(visitor);
        }
        ...
        {
            var t = await _repo.GetCompanyByLabel("Siemens");
            Assert.IsInstanceOf<Company>(t);
        }
    }
}
